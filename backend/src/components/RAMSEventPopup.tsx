...
export default function RAMSEventPopup({ event, onOpenProfile }: EventPopupProps) {
  ...
  return (
    <div className="p-4 w-[285px] sm:w-[310px] font-sans text-neutral-900 dark:text-neutral-100 select-none">
      ...
      {/* Full Profile & Dispatch Action Button */}
      {onOpenProfile && (
        <button
          onClick={(e) => {
            e.stopPropagation();
          ...
          className="mt-3 w-full py-2 px-3 rounded-lg font-mono text-xs font-bold uppercase tracking-wider text-white bg-blue-600 hover:bg-blue-700 active:bg-blue-800 transition-colors shadow-md flex items-center justify-center gap-1.5"
        >
          <UserCheck className="w-3.5 h-3.5" />
          <span>Full Profile & Dispatch</span>
